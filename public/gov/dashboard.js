...
document.addEventListener('DOMContentLoaded', async () => {
    if (window.showLocaloopLoader) {
        window.showLocaloopLoader('Syncing City Command Grid...');
    }
    ...
    document.getElementById('menuProfileRole').textContent = user.role || 'Gov Official';
...
async function fetchIssues() {
    try {
        ...
        // If official is not a demo account, only show real citizen reports (hide pre-seeded demo data)
        if (!isDemo) {
...
async function renderReports() {
    ...
    // If official with 0 issues in jurisdiction, show empty state immediately
    if (!isDemo && (!state.issues || state.issues.length === 0)) {
        container.innerHTML = `
            <div class="empty-state" style="text-align: center; padding: 36px 16px; color: var(--text-muted);">
                ...
                <div style="font-weight: 700; color: #1e293b; font-size: 15px; margin-bottom: 6px;">No Active Complaints</div>
                <p style="font-size: 13px; line-height: 1.5; margin: 0 0 16px; color: #64748b;">No active civic complaints reported in your jurisdiction yet. Newly filed resident reports will appear here.</p>
            </div>
        `;
        return;
    }
    ...
    navigator.geolocation.getCurrentPosition((position) => {
        ...
        // For demo official: if away from demo coords, show demo issues so they can test governance tools
        ...
        if (localIssues.length === 0) {
            container.innerHTML = `
                <div class="empty-state" style="text-align: center; padding: 36px 16px; color: var(--text-muted);">
                    ...
                    <div style="font-weight: 700; color: #1e293b; font-size: 15px; margin-bottom: 6px;">No Local Complaints</div>
                    <p style="font-size: 13px; margin: 0 0 16px; color: #64748b;">No active issues reported within 2km.</p>
                </div>
            `;
            return;
        }
        ...
        container.innerHTML = `
            <div style="padding: 10px 14px; font-size: 12px; color: var(--brand); font-weight: 600; display: flex; justify-content: space-between; align-items: center;">
                <span>${isDemo ? 'Demo Jurisdiction Reports' : 'Jurisdiction Reports'} (${localIssues.length})</span>
                ${isDemo ? '<span style="font-size: 10px; background: #fef3c7; color: #b45309; padding: 2px 6px; border-radius: 4px;">Demo Official</span>' : ''}
            </div>
        ...
        if (state.issues.length > 0) {
            ...
            container.innerHTML = `
                <div class="empty-state" style="text-align: center; padding: 36px 16px; color: var(--text-muted);">
                    ...
                    <div style="font-weight: 700; color: #1e293b; font-size: 15px; margin-bottom: 6px;">No Active Complaints</div>
                    <p style="font-size: 13px; margin: 0 0 16px; color: #64748b;">No active civic complaints reported yet. Citizen submissions will appear here.</p>
                </div>
            `;
        }
    });
